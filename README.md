...
| 插件 | 类型 | 说明 |
| --- | --- | --- |
| [`auto-allow-bash-plugin`](packages/auto-allow-bash-plugin) | hook + command | 通过 `PreToolUse` hook 拦截 Bash 与 PowerShell 调用，本地正则 + Haiku 双重判定，只读命令直接放行，写操作仍需人工确认；附带 `/auto-allow-bash-config` 命令用于配置 LLM 判定提示词 |
