...
yayaluoya 的 Claude Code 插件市场。单仓 monorepo 架构：插件源码放在 `packages/`，构建后打包输出到产品目录 `plugins/`，由根目录 `.claude-plugin/marketplace.json` 对外暴露。
...
```bash
pnpm install        # 安装依赖
pnpm run build      # turbo: 构建所有插件并打包到 plugins/
```

`build` 会对每个插件依次执行 `build`（如有构建步骤）和 `package`（把插件文件拷贝到 `plugins/<name>/`，并排除 `package.json`、`node_modules` 等工程文件）。
...
3. 在根 `.claude-plugin/marketplace.json` 的 `plugins` 数组里登记，`source` 指向 `./plugins/<插件名>`。
4. 运行 `pnpm run build`。
