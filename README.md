...
| 插件 | 类型 | 说明 |
| --- | --- | --- |
| [`auto-allow-bash-plugin`](packages/auto-allow-bash-plugin) | hook + command | 通过 `PreToolUse` hook 拦截 Bash 与 PowerShell 调用，先按权限模式短路（免确认模式直接交回系统），再本地正则 + Haiku 双重判定，只读命令直接放行，写操作仍需人工确认；附带 `/auto-allow-bash-config` 命令用于配置 LLM 判定提示词 |
...
| [`git-plugin`](packages/git-plugin) | command | 提供 `/commit`（按项目历史风格生成中文 commit 并安全提交）和 `/new-branch`（侦测分支命名规范、把中文名转成合规分支名并创建切换）命令 |
